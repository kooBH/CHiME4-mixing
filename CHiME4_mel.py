...
def process(idx):
    ...
    # save
    torch.save(torch_estim,os.path.join(output_root,'mel-'+str(n_mels),'measure','estim',target_category,target_id+'.pt'))
    torch.save(torch_noisy,os.path.join(output_root,'mel-'+str(n_mels),'measure','noisy',target_category,target_id+'.pt'))
    torch.save(torch_noise,os.path.join(output_root,'mel-'+str(n_mels),'measure','noise',target_category,target_id+'.pt'))
...
if __name__=='__main__': 
    ...
    for i in dirs :
        os.makedirs(os.path.join(output_root,'mel-'+str(n_mels),'measure','estim',i),exist_ok=True)
        os.makedirs(os.path.join(output_root,'mel-'+str(n_mels),'measure','noisy',i),exist_ok=True)
        os.makedirs(os.path.join(output_root,'mel-'+str(n_mels),'measure','noise',i),exist_ok=True)
